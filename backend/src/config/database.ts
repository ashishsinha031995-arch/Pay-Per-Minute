...
export function initDb() {
  ...
  const avatarsSetting = db.prepare('SELECT * FROM admin_settings WHERE key = ?').get('classic_avatars');
  if (!avatarsSetting) {
    ...
    db.prepare('INSERT INTO admin_settings (key, value) VALUES (?, ?)').run('classic_avatars', JSON.stringify(defaultAvatars));
  }
  ...
  if (consultantsCount.count === 0) {
    isNewlySeeded = true;
    console.log('[Database] SQLite is empty. Setting isNewlySeeded = true to protect MongoDB profiles from being overwritten.');
